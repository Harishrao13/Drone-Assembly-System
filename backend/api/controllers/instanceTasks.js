...
const validateSerial = async (req, res) => {
    ...
    try {
        ...
        const component = instance.components.find(c => c.partLabel === partLabel);
        const currentAssembledCount = component.serialNumbers.length;
        if (currentAssembledCount >= partQuantity) {
            return res.status(400).json({ msg: "Unit limit reached for component" });
        }
    ...
    try {
        const updatedInstance = await Instance.findOneAndUpdate(
            { _id: instanceId, "components.partLabel": partLabel },
        ...
        const updatedComponent = updatedInstance.components.find(c => c.partLabel === partLabel);
...
const updateProgressArchived = async (req, res) => {
    ...
    try{
        ...
        const updateProgess = await Instance.findOneAndUpdate( 
            { _id: instanceId }, 
            { status: 'archived', assembledOn: assembledOn, assembledBy: assembledBy},
            { new: true, runValidators: true }
        );
...
const getAssembledCounts = async (req, res) => {
    ...
    try {
        ...
        instance.components.forEach(component => {
            const partLabel = component.partLabel;
