  ...
  useEffect(() => {
    async function fetchData() {
      try {
        ...
        const assembledOnUTC = new Date(result.instance.assembledOn);
        const assembledOnIST = assembledOnUTC.toLocaleString('en-US', { timeZone: 'Asia/Kolkata' });
        result.instance.assembledOn = assembledOnIST;
      ...
      <div className="home flex flex-col rounded-md">
        ...
        <div className="flex justify-start text-left">
          <div className="flex flex-col space-y-4 mx-0">
            ...
            <Label htmlFor="assembledBy" className="text-lg font-semibold">Assembled By: {data.assembledBy || 'N/A'}</Label>
            ...
            {data.components.map((component, index) => (
              <div key={index} className="flex flex-col mx-2 space-y-2">
                <Label htmlFor="componentName" className="text-lg font-semibold">{component.partLabel}</Label>
                <ul className="list-disc list-inside">
                  {component.serialNumbers.map((serialNumber, snIndex) => (
                    <li key={snIndex} className="ml-4">{serialNumber}</li>
                  ))}
                </ul>
              </div>
            ))}
